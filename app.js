...
function displayTest(diff) {
  ...
  words = randomWords(diff);
  words.forEach((word, i) => {
    let wordSpan = document.createElement('span');
    wordSpan.setAttribute("id", `word-${i}`);
    wordSpan.className = 'word';
    
    for (let j = 0; j < word.length; j++) {
      let letterSpan = document.createElement('span');
      letterSpan.textContent = word[j];
      letterSpan.setAttribute("id", `letter-${i}-${j}`);
      letterSpan.className = 'letter';
      wordSpan.appendChild(letterSpan);
    }
    
    testItem.appendChild(wordSpan);
    testItem.appendChild(document.createTextNode(' ')); // Add space between words
  });
...
function updateCursor() {
  ...
  let currentLetter = document.getElementById(`letter-${currentWordIndex}-${currentLetterIndex}`);
  if (currentLetter) {
    currentLetter.classList.add('cursor');
  } else {
    let currentWord = document.getElementById(`word-${currentWordIndex}`);
    if (currentWord) {
      let cursorSpan = document.createElement('span');
      cursorSpan.classList.add('cursor', 'letter');
      cursorSpan.textContent = '\u00A0'; // Non-breaking space
      currentWord.appendChild(cursorSpan);
    }
  }
}
...
function checkInput() {
  ...
  for (let i = 0; i < Math.max(inputWords.length, words.length); i++) {
    ...
    const wordToCheck = words[i] ? words[i].trim() : ''; // Remove trailing space
    const inputWord = inputWords[i] || '';

    if (wordSpan) {
      const letters = wordSpan.children;
      let allCorrect = true;

      // Check each letter
      for (let j = 0; j < wordToCheck.length; j++) {
        const letterSpan = letters[j];
        if (j < inputWord.length) {
          if (inputWord[j] === wordToCheck[j]) {
...
function displayScore() {
  ...
  console.log(wordsCorrect, wordsSubmitted);
